...
def build_u2i_matrix(user_item_score_data_path, item_name_data_path, write_file=False):
    ...
    # 读打分文件
    user_to_rating = {}  # user_id: [rate1, rate2, ...]
...
def find_similar_item(user_to_rating):
    item_to_vector = {}  # item_id: [user_rate1, user_rate2, ...]
    total_user = len(user_to_rating)  # 943
    for user, user_rating in user_to_rating.items():  # user: 1, user_rating: [5, 3, 0, ...]
        for moive_id, score in enumerate(user_rating):  # moive_id: 1, score: 5
            moive_id += 1
    ...
    # 复用一下下方的相似度计算方法
    # 计算item之间的相似度, item_id: [[similar_item_id, similarity], ...]
    return find_all_similar_user(item_to_vector)


# 依照user对item的打分判断user之间的相似度
def find_similar_user(user_to_rating):
    user_to_similar_user = {}  # user_id: [[similar_user_id, similarity], ...]
    score_buffer = {}
...
def find_all_similar_user(user_to_rating):
    user_to_similar_user = {}
    score_buffer = {}
    user_to_rating = list(user_to_rating.items())
    for i in range(len(user_to_rating)):
        user_a, ratings_a = user_to_rating[i]
        similar_user = []
        for j in range(i + 1, len(user_to_rating)):
            user_b, ratings_b = user_to_rating[j]
            similarity = cosine_distance(np.array(ratings_a), np.array(ratings_b))  # 计算余弦相似度
            score_buffer["%d_%d" % (user_a, user_b)] = similarity
            similar_user.append([user_b, similarity])
        similar_user = sorted(similar_user, reverse=True, key=lambda x: x[1])
        user_to_similar_user[user_a] = similar_user
...
def item_cf(user_id, item_id, similar_items, user_to_rating, topn=10):
    ...
    for similar_item, similarity in similar_items[item_id][:topn]:
        rating_by_similiar_item = user_to_rating[user_id][similar_item - 1]  # 相似电影的用户打分
...
# 对于一个用户做完整的item召回
def movie_recommand(user_id, similar_user, similar_items, user_to_rating, item_to_name, topn=10, method="user_cf"):
    # 当前用户还没看过的所有电影id
    ...
    for item_id in unseen_items:
        ...
        # item_cf打分
            ...
            score = item_cf(user_id, item_id, similar_items, user_to_rating)  # 用户user_id对电影unseen_item相似的topn电影的打分
...
if __name__ == "__main__":
    ...
    # user-cf
    similar_user = find_all_similar_user(user_to_rating)
    ...
    #     res = item_cf(user_id, item_id, similar_user, user_to_rating)
    #     print(res)
    ...
    user_id = int(input("输入用户id："))
    recommands = movie_recommand(user_id, similar_user, similar_items, user_to_rating, item_to_name)
    print("基于 User cf 获得的结果")
    for recommand, score in recommands:
        print("%.4f\t%s" % (score, recommand))

    recommands = movie_recommand(user_id, similar_user, similar_items, user_to_rating, item_to_name, method="item_cf")
    print("基于 Item cf 获得的结果")
    for recommand, score in recommands:
        print("%.4f\t%s" % (score, recommand))
